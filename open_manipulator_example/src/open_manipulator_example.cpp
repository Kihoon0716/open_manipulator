  ...
  if( task == CHECK_OBJECT_POSE ) {
	  ...
	  for(int i = 0 ; i < msg->poses.size() ; i++){
		if( !( ( msg->poses[i].position.x == 0 ) && ( msg->poses[i].position.y == 0 ) && ( msg->poses[i].position.z == 0 ) ) ) {
			...
			ROS_INFO("%d _  %.3f, %.3f, %.3f _  %.3f, %.3f, %.3f  ",i, object_x, object_y, object_z, centroid_pose[i].position.x, centroid_pose[i].position.y, centroid_pose[i].position.z );
...
void OM_EXAMPLE::process(void)
{
  ...
  switch (task)
  {
	...
	goalPose.at(2) = object_z + 0.065 ; //z
	setTaskSpacePath(goalPose, 2);
        sleep(2.2);
